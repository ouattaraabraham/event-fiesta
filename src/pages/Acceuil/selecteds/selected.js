...
export default function Selected({NameSelect,selected,handlerChange,data}) {

  return (
    <div className="relative 	 w-full		">
      <Listbox value={selected} onChange={handlerChange}>
        <div className="relative mt-1">
        <Listbox.Label>{NameSelect}</Listbox.Label>
          <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
            <span className={selected?"block truncate":"block truncate opacity-60"}>{selected?selected.name:data[0].label}</span>
